...
window.addEventListener('load', function() {
  ...
  if ( /* checking user device type based on regexp */
  ...
  {
    // Difference, in milliseconds, between animation loop iterations
    let deltaTime = currentTime - lastTime

    // rAF calls are paused when running in background tabs
    if (deltaTime > 100) deltaTime = 100              // minimum 10 fps

    lastTime = currentTime                            // Reassigning temporary positions
    ctx.clearRect(0, 0, canvas.width, canvas.height)  // Clearing the play area before rendering
