...
const firefox = /Firefox/i.test((window.navigator && window.navigator.userAgent) || '')
const speed = firefox ? 0.05 : 0.015
...
class Creature extends Chain
{
  ...
  {
    if (!this.follow) {
      const t = simplex.noise3D(this.target[0] * 0.005, this.target[1] * 0.005, frameCount * 0.005) * TAU
  ...
  {
    this.setPosition([
      lerp(this.position[0], this.target[0], speed),
      lerp(this.position[1], this.target[1], speed)
    ])
  ...
  drawSegment(x1, y1, x2, y2, h, n, tn, i)
  {
    ...
    const lineLength = 1.35 * arcWidth,
      nubWidth = 0.35 * arcWidth,

      lineLeftX = x1 + lineLength * cos(tn + 0.85 + n),
      lineLeftY = y1 + lineLength * sin(tn + 0.85 + n),

      lineRightX = x1 + lineLength * cos(tn - 0.85 - n),
      lineRightY = y1 + lineLength * sin(tn - 0.85 - n)
