import{A,E as G,C as I}from"./main.esm-DqBob0QU.js";import{d as w,i as T,o as y,c as k,a as d,b as m,u as R,F as E,r as h,s as p,_ as M,e as L,w as g}from"./index-CwHQ-y8K.js";const F=w({__name:"Example8.demo",emits:["log"],setup(v,{emit:c}){const a=c,s=T("onGridReady"),o=h([{id:1,firstName:"Иван",salary:1e5},{id:2,firstName:"Мария",salary:15e4},{id:3,firstName:"Алексей",salary:2e5}]);let f=4;const S=p([{field:"id",headerName:"ID",width:80},{field:"firstName",headerName:"Имя"},{field:"salary",headerName:"Зарплата",valueFormatter:e=>e.value?`${e.value.toLocaleString("ru-RU")} ₽`:""}]),D=p({sortable:!0,resizable:!0,filter:!0});function N(e){return String(e.data.id)}let n=null;function $(e){n=e.api,e.api.sizeColumnsToFit(),s&&s()}function b(){if(!n)return;const e={id:f++,firstName:`Новый ${f-1}`,salary:Math.round(Math.random()*1e5+5e4)};o.value=[...o.value,e],n.applyTransaction({add:[e]}),a("log",`Добавлена строка #${e.id}: ${e.firstName}, зарплата ${e.salary}`)}function x(){if(!n)return;const e=n.getSelectedRows();if(!e.length){a("log","Нет выбранных строк для обновления");return}const r=e.map(t=>{let i=t.firstName;i.includes("(ред)")||(i=`${i} (ред)`);const u=Math.round(t.salary*(.8+Math.random()*.4));return{...t,firstName:i,salary:u}}),l=new Set(r.map(t=>t.id));o.value=o.value.map(t=>l.has(t.id)?r.find(i=>i.id===t.id):t),n.applyTransaction({update:r}),a("log",`Обновлено ${r.length} строк(и)`)}function C(){if(!n)return;const e=n.getSelectedRows();if(!e.length){a("log","Нет выбранных строк для удаления");return}const r=new Set(e.map(l=>l.id));o.value=o.value.filter(l=>!r.has(l.id)),n.applyTransaction({remove:e}),a("log",`Удалено ${e.length} строк(и)`)}function _(){if(!n)return;const e=n.getDisplayedRowCount(),r=n.getSelectedRows().length,l=o.value.reduce((i,u)=>i+u.salary,0),t=e?Math.round(l/e):0;a("log",`📊 Всего строк: ${e}, выбрано: ${r}`),a("log",`💰 Сумма зарплат: ${l.toLocaleString("ru-RU")} ₽, средняя: ${t.toLocaleString("ru-RU")} ₽`)}return(e,r)=>(y(),k(E,null,[d("div",{class:"controls"},[d("button",{onClick:b},"➕ Добавить"),d("button",{onClick:x},"✏️ Обновить выбранного"),d("button",{onClick:C},"🗑️ Удалить выбранных"),d("button",{onClick:_},"📊 Показать информацию")]),m(R(A),{style:{flex:"1",width:"100%"},rowData:o.value,columnDefs:S.value,defaultColDef:D.value,getRowId:N,rowSelection:"multiple",animateRows:!0,onGridReady:$},null,8,["rowData","columnDefs","defaultColDef"])],64))}}),U=M(F,[["__scopeId","data-v-740cf570"]]),V=`<template>
  <div class="controls">
...
`,z="Транзакции (производительность)",B="Вместо полной перерисовки таблицы при каждом изменении, транзакции (applyTransaction) обновляют только изменённые строки. Это критически важно для производительности при работе с большими объёмами данных (сотни тысяч строк).",O=w({__name:"Example8",setup(v){const c=h([]);function a(s){c.value.push(`[${new Date().toLocaleTimeString()}] ${s}`)}return(s,o)=>(y(),L(G,{title:z,description:B,code:R(V),language:"html",onLog:a},{console:g(()=>[m(I,{logs:c.value},null,8,["logs"])]),default:g(()=>[m(U,{onLog:a})]),_:1},8,["code"]))}});export{O as default};
