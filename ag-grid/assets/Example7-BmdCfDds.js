...
`,J="Сохранение состояния",L="Таблица сохраняет данные, ширину колонок, порядок, сортировку и фильтры в localStorage. При перезагрузке страницы состояние восстанавливается. Также можно добавлять, удалять и редактировать записи.",P=y({__name:"Example7",setup(h){const s=x([]);function l(d){s.value.push(`[${new Date().toLocaleTimeString()}] ${d}`)}return(d,u)=>(D(),Y(O,{title:J,description:L,code:S(H),language:"html",onLog:l},{console:v(()=>[p(V,{logs:s.value},null,8,["logs"])]),default:v(()=>[p(B,{onLog:l})]),_:1},8,["code"]))}});export{P as default};
