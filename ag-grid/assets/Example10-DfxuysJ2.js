...
`,U="Кастомный фильтр",j="Vue-компонент фильтра по диапазону возраста. Показывает реализацию интерфейса IFilter: isFilterActive, doesFilterPass, getModel, setModel, afterGuiAttached. Используйте фильтр в колонке или пресеты сверху.",q=N({__name:"Example10",setup(g){const s=f([]);function r(t){s.value.push(`[${new Date().toLocaleTimeString()}] ${t}`)}return(t,l)=>(b(),M(D,{title:U,description:j,code:x(T),language:"html",onLog:r},{console:C(()=>[y(R,{logs:s.value},null,8,["logs"])]),default:C(()=>[y(L,{onLog:r})]),_:1},8,["code"]))}});export{q as default};
