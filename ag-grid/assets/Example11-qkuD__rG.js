...
<\/script>
`,Q="Кастомный cell editor",U='Vue-компонент редактора ячейки с автодополнением. Реализует методы getValue, isPopup, isCancelBeforeStart, isCancelAfterEnd. Дважды кликните на колонку "Отдел", чтобы открыть редактирование.',ee=E({__name:"Example11",setup(g){const s=f([]);function a(o){s.value.push(`[${new Date().toLocaleTimeString()}] ${o}`)}return(o,l)=>(i(),N(V,{title:Q,description:U,code:R(K),language:"html",onLog:a},{console:w(()=>[D(I,{logs:s.value},null,8,["logs"])]),default:w(()=>[D(J,{onLog:a})]),_:1},8,["code"]))}});export{ee as default};
